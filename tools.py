    ...
    async def execute(
        ...
        for stream in streams:
            ...
            if normalized_keyword and normalized_keyword not in gname.lower() and normalized_keyword not in gid.lower():
                continue
            items.append({"group_id": gid, "group_name": gname or None, "platform": str(getattr(stream, "platform", "") or ""), "last_active_time": getattr(stream, "last_active_time", None)})
            if len(items) >= normalized_limit:
                break

        return True, {"action": "send_to_list_groups", "query": {"platform": normalized_platform or None, "name_keyword": normalized_keyword or None, "limit": normalized_limit}, "total": len(items), "groups": items, "hint": "拿到候选后调用 send_to(target_type='group', group_id=<选中的>)。"}
...
class ListUsersTool(BaseTool):
    ...
    async def execute(
        ...
        for person in persons:
            ...
            if not uid:
                continue
            if normalized_keyword and not (normalized_keyword in uid.lower() or normalized_keyword in nickname.lower() or normalized_keyword in cardname.lower()):
                continue
            items.append({"user_id": uid, "nickname": nickname or None, "cardname": cardname or None, "platform": str(getattr(person, "platform", "") or ""), "last_interaction": getattr(person, "last_interaction", None), "interaction_count": getattr(person, "interaction_count", None)})
            if len(items) >= normalized_limit:
                break

        return True, {"action": "send_to_list_users", "query": {"platform": normalized_platform or None, "name_keyword": normalized_keyword or None, "limit": normalized_limit}, "total": len(items), "users": items, "hint": "拿到候选后调用 send_to(target_type='private', user_id=<选中的>)。"}
...
class LookupUsersTool(BaseTool):
    ...
    async def execute(
        ...
        return True, {"action": "send_to_lookup_users", "query": {"keyword": normalized_keyword, "platform": normalized_platform or None, "limit": normalized_limit}, "total": len(combined), "users": combined, "hint": "拿到候选后调用 send_to(target_type='private', user_id=<选中的>)。"}
...
class SendToStreamContextTool(BaseTool):
    ...
    async def execute(self, stream_identifier: Annotated[str, "目标聊天流名称、索引号或 stream_id"], message_count: Annotated[int, "消息数量，默认 20，最多 200"] = 20) -> tuple[bool, str]:
        ...
        if ident.isdigit() and len(ident) <= 3:
            ...
            matched = [r for r in records if ident.lower() in _build_stream_title(r).lower() or ident == r.stream_id]
            if not matched:
...
class SendToDailyMemoryTool(BaseTool):
    ...
    async def execute(self, stream_identifier: Annotated[str, "目标群标识：群名、索引号或群号"], days: Annotated[int, "查询最近天数，默认 1"] = 1, date_filter: Annotated[str, "指定日期 YYYY-MM-DD"] = "") -> tuple[bool, str]:
        ...
        if ident.isdigit() and len(ident) <= 3:
            ...
            if 0 <= idx < len(records):
                target = records[idx]
        if target is None:
            target = next((r for r in records if r.chat_type == "group" and r.target_id and str(r.target_id) == ident), None)
        if target is None:
            matched = [r for r in records if r.chat_type == "group" and ident.lower() in _build_stream_title(r).lower()]
            if len(matched) == 1:
                target = matched[0]
            elif len(matched) > 1:
                return False, "\n".join([f"找到 {len(matched)} 个匹配的群，请更精确："] + [f"{idx}. {_build_stream_title(r)} (群号 {r.target_id or '未知'})" for idx, r in enumerate(matched[:6], start=1)])
        if target is None:
        ...
        if date_filter.strip():
            ...
            records_daily = [r for r in [await get_memory(self.plugin, target.stream_id, requested)] if r is not None]
        else:
            records_daily = await list_recent_memories(self.plugin, target.stream_id, max(1, min(int(days), max_query_days)))
        if not records_daily:
            return False, f"{_build_stream_title(target)} 没有可用短期记忆"

        lines = [f"群聊: {_build_stream_title(target)}", f"群号: {target.target_id or '未知'}", f"平台: {target.platform or 'unknown'}", ""]
        for record in records_daily:
            lines.extend([f"── {record.memory_date} ──", f"消息总数: {record.message_count}    更新于: {record.updated_at}", "", record.summary, ""])
        return True, "\n".join(lines).rstrip()
...
class SendToFindStreamTool(BaseTool):
    ...
    async def execute(self, identifier: Annotated[str, "目标流标识：名称、索引号、QQ号、群号或 stream_id"], chat_type_hint: Annotated[str, "可选消歧：private/group"] = "") -> tuple[bool, str]:
        ...
        if len(ident) == 64 and all(c in "0123456789abcdef" for c in ident.lower()):
            ...
            if len(matched) == 1:
                ...
                return False, "\n".join([f"找到 {len(matched)} 个匹配的聊天流，请进一步精确："] + [f"{idx}. {_build_stream_title(r)} [{r.platform}:{r.chat_type}] (id {r.target_id or '未知'})" for idx, r in enumerate(matched[:8], start=1)])
        if target is None:
        ...
        try:
            ...
            in_memory = False
        lines = ["找到目标流，可用于 send_to_relay_intent 或 send_to：", "", "【下一步调用参数建议】", f"- target_stream_id: \"{target.stream_id}\"", f"- target_platform: \"{target.platform or 'qq'}\"", f"- target_user_id: \"{target.target_id if target.chat_type == 'private' else ''}\"", f"- target_group_id: \"{target.target_id if target.chat_type == 'group' else ''}\"", "", "【目标流详情】", f"名称: {_build_stream_title(target)}", f"platform: {target.platform or 'unknown'}", f"chat_type: {target.chat_type or 'unknown'}", f"is_in_memory: {bool(in_memory)}", f"updated_at: {target.updated_at or 'unknown'}", "", "【当前摘要】", target.summary or "(暂无摘要)"]
        return True, "\n".join(lines)
