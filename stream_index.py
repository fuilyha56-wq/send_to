"""send_to 插件的摘要存储与 reminder 同步逻辑。

整合自原 context_bridge/service.py，配置类改为 SendToConfig，
其余行为保持不变；其中：
  - ACTOR_REMINDER_BUCKET / ACTOR_REMINDER_NAME 保持 ``actor`` / ``跨聊天流上下文摘要``，
    避免影响已注入到 actor bucket 的现有 chatter prompt 行为。
  - LLM 请求名称前缀 ``send_to_auto_summary_*``。
"""
...
async def collect_message_for_auto_summary(
    ...
    *,
    ...
    batch_size = max(1, int(config.index.auto_summary_batch_size))
...
def build_actor_reminder(
    ...
    lines = [
        ...
        "1. 跨流转告：当你需要从当前流切换到另一个流执行任务时，请先调用 tool-send_to_find_stream 找到目标流，然后调用 action-send_to_relay_intent 将你的意图、关键原话和执行提示推送过去。目标流的你（Chatter）会收到这条转告并自然地继续对话。",
        "2. 自动摘要：插件会自动更新各流摘要。只有当摘要偏离事实或有关键新信息（如约定的私聊、跨流待办）需要立刻记录时，才调用 action-send_to_update_stream_summary。",
    ...
    lines.extend([
        "",
        "可用工具：",
        "- tool-send_to_get_stream_context: 查询【其他聊天流】的原始聊天记录（不能用于本流，本流上下文已直接可见）",
        "- tool-send_to_get_daily_memory: 跨群查询某群最近几天（默认含今天）的短期记忆，用以补全摘要细节",
        "- tool-send_to_find_stream: 把名字/索引/QQ 号反查成 stream_id 等元组，供 send_to_relay_intent 使用",
        "- action-send_to_relay_intent: 把一段转告便条直接推送到目标流，并在目标流冷态时主动唤醒",
    ])
...
async def sync_actor_reminder(
    ...
    if not config.index.inject_summary_reminder:
        return ""
    ...
    reminder_text = build_actor_reminder(
        ...
        visible_stream_limit=config.index.visible_stream_limit,
