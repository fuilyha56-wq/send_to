...
async def _list_candidate_targets(
    ...
    for stream in streams:
        ...
        if chat_type == "group":
            ...
            # 私聊取 person_id 末段（这里用 stream_id 关联）
            target_id = sid
        else:
            continue
...
async def _format_messages(stream_id: str, limit: int) -> list[str]:
    ...
    lines: list[str] = []
    for msg in msgs:
        role = "[bot]" if getattr(msg, "sender_role", "") == "bot" else "[人]"
        text = str(
...
async def _llm_decide(
    ...
    for idx, cand in enumerate(candidates, 1):
        preview = await _format_messages(
            cand["stream_id"], cfg.target_preview_messages
        )
