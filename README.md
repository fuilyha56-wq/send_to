...
支持流式模型调用，可适配仅提供流式响应的模型供应商；自动摘要、每日短期记忆和串门决策会完整消费流式响应后再处理结果。
...
| `[index]` | 跨流摘要、summary reminder、摘要长度、批次和实时数据保留天数 |
...
`index.retention_days` 默认是 `3`。超过该期限的跨流摘要和待摘要消息会在插件启动或下次读取时删除，后续聊天会重新积累并生成摘要。该设置不删除 `daily_memory` 每日短期记忆；设为 `0` 可禁用过期清理。
